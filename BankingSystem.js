  ...
  createAccount(name, pin) {
    try {
      ...
      if (this.isIdTaken(id)) {
        console.log('ID already exists. Generating a new ID.');
        this.createAccount(name, pin);
        return;
      }

      if (pin < 1000) {
        console.log("PIN must be 4-digits and first digit of PIN can't be zero");
        return;
      } else if (pin > 9999) {
    ...
    try {
      ...
      if (account && account.pin === pin) {
        if (account.withdraw(amount)) {
          ...
          console.log(`Withdrawal successful. New balance: ${account.balance} KM`);
        } else {
    ...
    try {
      ...
      if (fromAccount && fromAccount.pin === fromPin && toAccount) {
        if (fromAccount.balance >= amount) {
          ...
          console.log(`Transfer successful. New balance: ${fromAccount.balance} KM`);
        } else {
          console.log('Insufficient funds.');
        }
      } else {
      ...
      rl.question('Enter your choice: ', choice => {
        switch (choice) {
            ...
            rl.question('Enter your first name: ', firstName => {
              rl.question('Enter your surname: ', surname => {
                ...
                rl.question('Enter a 4-digit PIN: ', pin => {
                  rl.question('Confirm account creation? (yes/no): ', confirm => {
          ...
          case '5':
            rl.question('Enter your account ID: ', id => {
              rl.question('Enter your PIN: ', pin => {
                this.viewTransactions(parseInt(id), parseInt(pin));
                this.mainMenu();
              });
            });
            break;
